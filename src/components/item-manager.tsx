...
export function ItemManager({ items, currency }: { items: CatalogItem[]; currency: string }) {
  ...
  return <>
    ...
    <div className="grid gap-3 md:hidden">{filtered.length === 0 ? <div className="rounded-xl border border-[var(--rule)] bg-white p-6 text-center text-sm text-[var(--muted)]">No catalog products found.</div> : filtered.map((item) => <article key={item.id} className="rounded-xl border border-[var(--rule)] bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="font-black">{item.name}</h3><p className="mt-1 whitespace-pre-line text-xs leading-5 text-[var(--muted)]">{item.description || "—"}</p></div><div className="flex shrink-0 gap-1"><button type="button" className="rounded-lg p-2 hover:bg-slate-100" onClick={() => begin(item)} aria-label="Edit product"><Pencil size={16}/></button><button type="button" className="rounded-lg p-2 text-red-700 hover:bg-red-50" onClick={() => setDeleting(item)} aria-label="Delete product"><Trash2 size={16}/></button></div></div><div className="mt-3 flex items-end justify-between border-t border-slate-100 pt-3"><div><p className="text-xs text-[var(--muted)]">Unit</p><p className="mt-1 text-sm font-semibold">{item.default_unit}</p></div><div className="text-right"><p className="text-xs text-[var(--muted)]">Default rate</p><p className="mt-1 font-black money">{currency} {Number(item.default_rate).toFixed(2)}</p></div></div></article>)}</div>
