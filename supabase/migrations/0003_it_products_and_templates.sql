-- JasimFlow v3 incremental upgrade
-- 1) CCTV/IT-friendly line items: separate Product and Description/Specification.
-- 2) New line-item default unit is pcs.
-- 3) Six persisted premium document templates.
-- Run AFTER 0001_init.sql and 0002_invoice_customization_and_logo_storage.sql.
...
set product_name = coalesce(nullif(btrim(product_name), ''), nullif(btrim(description), ''), 'Product'),
    description = case when product_name is null or btrim(product_name) = '' then '' else description end
...
alter table public.documents add column if not exists template_style text not null default 'tech';
update public.documents set template_style='tech' where template_style is null or template_style not in ('executive','tech','minimal','graphite','emerald','copper');
...
begin
  ...
  insert into public.documents (
    ...
    v_status,
    coalesce(nullif(p_payload->>'template_style',''),'tech'),
  ...
  loop
    -- Backward-compatible fallback lets older/demo payloads continue working.
    v_product := coalesce(nullif(btrim(v_item->>'product_name'),''), nullif(btrim(v_item->>'description'),''));
    ...
    values (
      ...
      v_product,
      case when nullif(btrim(v_item->>'product_name'),'') is null then '' else coalesce(btrim(v_item->>'description'),'') end,
...
begin
  ...
  update public.documents set
    ...
    template_style=coalesce(nullif(p_payload->>'template_style',''),template_style),
